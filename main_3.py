...
def main(yolo):
    ...
    while True:
        ...
        for i, cl in enumerate(current_nums):
            if cl > 0:
                cv2.putText(frame, LABELS[i] + "=" + str(cl), (x1, y1), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255),1)  # 当前帧各类别数量
                y1 = y1 + 20
        ...
        for i, det in enumerate(detections):
            ...
            cv2.putText(frame, score + '%', (int(bbox[0]), int(bbox[1]) + 10), cv2.FONT_HERSHEY_DUPLEX, 0.3, (255, 255, 255), 1)
        ...
        for track in tracker.tracks:
            ...
            cv2.putText(frame, str(track.track_id)+' '+LABELS[track.class_id], (int(bbox[0]), int(bbox[1])-5), cv2.FONT_HERSHEY_DUPLEX, 0.4, color, 1)
